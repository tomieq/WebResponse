...
public extension WebRequest {
    ...
    func post(url: String, body: Encodable, headers: [String: String]? = nil) async -> WebResponse<T> {
        await run(url: url, method: "POST", body: body.data, headers: headers)
    }
    ...
    func post(url: String, body: Data, headers: [String: String]? = nil) async -> WebResponse<T> {
        await run(url: url, method: "POST", body: body, headers: headers)
    }
